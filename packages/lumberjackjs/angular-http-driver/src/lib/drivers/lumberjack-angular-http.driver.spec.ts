...
function createHttpDriverLog(
  { log, formattedLog }: LumberjackDriverLog<HttpDriverPayload>,
  origin = 'TEST_MODULE'
...
describe(LumberjackAngularHttpDriver.name, () => {
  let httpDriver: LumberjackDriver<HttpDriverPayload>;
  ...
  beforeEach(() => {
    ...
    [httpDriver] = TestBed.inject(lumberjackDriverToken) as unknown as LumberjackDriver<HttpDriverPayload>[];
  ...
  ] as ReadonlyArray<[LumberjackLogLevel, (driver: LumberjackDriver<HttpDriverPayload>) => (driverLog: LumberjackDriverLog<HttpDriverPayload>) => void]>)(
